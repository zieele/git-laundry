<div id="update-modal-{{ $item->getKey() }}"
    ...
    <form action="{{ route('outlet.update', $item->getKey()) }}" method="post" class="bg-white w-96 flex flex-col rounded-xl text-gray-600 p-4">
        ...
        <div>
            ...
            <input class="mb-4 mt-1 outline-none duration-300 w-full bg-white focus:bg-blue-50 border border-gray-600 rounded-lg px-3 py-2" type="text" name="nama" id="nama" autocomplete="off" required placeholder="Masukan Nama" value="{{ $item->nama }}">
    
            <label class="font-semibold text-gray-600" for="alamat">Alamat</label>
            <input class="mb-4 mt-1 outline-none duration-300 w-full bg-white focus:bg-blue-50 border border-gray-600 rounded-lg px-3 py-2" type="text" name="alamat" id="alamat" autocomplete="off" required placeholder="Tulis Alamat" value="{{ $item->alamat }}">
    
            <label class="font-semibold text-gray-600" for="tlp">No. Tlp</label>
            <input class="mb-4 mt-1 outline-none duration-300 w-full bg-white focus:bg-blue-50 border border-gray-600 rounded-lg px-3 py-2" type="year" name="tlp" id="tlp" autocomplete="off" required placeholder="Tulis No. Tlp" value="{{ $item->tlp }}"  onkeypress="return num(event)">
        </div>
