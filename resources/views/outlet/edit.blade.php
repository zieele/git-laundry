<div id="modalEdit{{ $item->id }}" class="hidden z-50 bg-black bg-opacity-30 fixed flex justify-center items-center w-screen h-screen transform -translate-y-20 lg:-translate-x-64 xl:-translate-x-96">
    <form class="rounded-xl bg-blue-50 overflow-hidden text-gray-700" action="{{ route('outlet.update',$item->id) }}" method="POST">
        ...
        <div class="px-4 flex flex-col justify-center items-end">
          ...
          <div class="my-4 float-right">
            <button type="button" id="closeEdit{{ $item->id }}" class="shadow-md hover:shadow-xl px-2 py-1 rounded-lg bg-red-400 text-white hover:bg-red-300 duration-300 font-semibold transform hover:-translate-y-1">
                <span>Cancel</span>
            </button>
...
@push('script')
<script>
    document.getElementById("closeEdit{{ $item->id }}").onclick = function() {
        document.getElementById("modalEdit{{ $item->id }}").classList.toggle("hidden");
        document.getElementById("body").classList.toggle("overflow-y-hidden");
    }
    document.getElementById("showEdit{{ $item->id }}").onclick = function() {
        document.getElementById("modalEdit{{ $item->id }}").classList.toggle("hidden");
        document.getElementById("body").classList.toggle("overflow-y-hidden");
    }
</script>
