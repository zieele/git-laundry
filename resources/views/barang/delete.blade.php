<div id="delete-modal-{{ $item->getKey() }}"
    ...
    <form action="{{ route('barang.destroy', $item->getKey()) }}" method="post" class="bg-white w-96 flex flex-col rounded-xl text-gray-600">
        @csrf
        @method('DELETE')
        ...
        <div class="mx-4"><span>Jika anda menghapus data <span class="text-red-400 font-semibold lowercase">{{strtok( $item->barang , " ")}}</span>, maka data akan dihilangkan dari database.</span></div>
